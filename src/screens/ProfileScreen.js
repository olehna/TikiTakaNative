...
export class ProfileScreen extends Component {
  ...
  getUser = async () => {
    ...
    const userInfo = userList.find(
      (elem) => elem.email['stringValue'] === userEmail
    );
    // console.log(userList[userList.length - 2].email.stringValue);
    // console.log(userList);
    this.setState({
      userId: userInfo.userId.stringValue,
      userEmail: userInfo.email.stringValue,
      userName: userInfo.userName.stringValue,
      userGames: userInfo.games.integerValue,
      userRightAnswers: userInfo.rightAnswers.integerValue,
      userFirstName: userInfo.firstName.stringValue,
      userLastName: userInfo.lastName.stringValue,
    });
    // console.log(userList.find(elem => elem.email['stringValue'] === userEmail));
    // console.log('userlist => ', this.state.users[0].userId["stringValue"]);
    // const currentUser = await userList.find(elem => { elem.userId["stringValue"] === userId })
    // console.log(currentUser);
    // return userList
  };
  // const currentUser = userList.find(elem => { elem.userId["stringValue"] === user })
  // console.log(currentUser);
  // return users
  render() {
      ...
      <LinearGradient colors={['#de3c5e', '#7ebead']} style={{ flex: 1 }}>
        <View style={styles.center}>
          <View style={styles.avaAndName}>
            ...
            <Text style={styles.rating}>{this.state.userFirstName}</Text>
          </View>
          <View style={styles.statisticsContainer}>
            <View style={styles.progressBar}></View>
            <View style={styles.statistics}>
              <View style={{ alignItems: 'center' }}>
                <Text style={styles.title}>Правильные ответы</Text>
                <Text style={styles.number}>{this.state.userRightAnswers}</Text>
              </View>
              <View style={{ alignItems: 'center' }}>
                <Text style={styles.title}>Игры</Text>
                <Text style={styles.number}>{this.state.userGames}</Text>
              </View>
            </View>
          </View>
        </View>
